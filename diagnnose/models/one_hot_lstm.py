...
class OneHotLSTM(LanguageModel):
    ...
    def __init__(
        ...
        for l in range(self.num_layers):
            ...
            self.weight[l] = torch.cat(
                (
                    state_dict[f"{rnn_name}.weight_hh_l{l}"],
                    state_dict[f"{rnn_name}.weight_ih_l{l}"],
                ),
                dim=1,
            )
            ...
            if f"{rnn_name}.bias_ih_l{l}" in state_dict:
                # (4*hidden_size,)
                self.bias[l] = (
                    state_dict[f"{rnn_name}.bias_ih_l{l}"]
    ...
    def forward_step(
        self, l: int, inp: Tensor, prev_hx: Tensor, prev_cx: Tensor
    ) -> NamedArrayDict:
        # (4*hidden_size,)
    ...
    @overrides
    def forward(
        self, idx: str, prev_activations: FullActivationDict, compute_out: bool = True
